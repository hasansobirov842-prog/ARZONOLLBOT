...
PAYMENT_OWNER = os.getenv("PAYMENT_OWNER", "RAKHMONVA O")
...
async def topup_page(update, context):
    ...
    card = PAYMENT_CARD if PAYMENT_CARD else "KARTA SOZLANMAGAN"
    ...
    await update.message.reply_text(
        ...
        f"💳 Karta:\n{card}\n\n"
        f"👤 Karta egasi:\n{PAYMENT_OWNER}\n\n"
        "Avval qancha to'lov qilganingizni yozing.\n"
        "Masalan: 10000",
        reply_markup=back_keyboard()
    )
...
async def process_topup_amount(update, context):
    ...
    await update.message.reply_text(
        ...
        "👇 **🧾 Chek yuborish** tugmasini bosing yoki "
...
async def receipt_button(update, context):
    query = update.callback_query
    await query.answer()
    ...
    await query.message.reply_text(
        ...
        "Chek adminga avtomatik yuboriladi."
    )
...
async def receive_receipt(update, context):
    ...
    await update.message.reply_text(
        ...
        "Tasdiqlangandan keyin pul balansingizga avtomatik tushadi.",
        reply_markup=main_menu()
    )
...
async def callback_handler(update, context):
    ...
    if data == "topup":
        ...
        await query.message.reply_text(
            ...
            f"💳 Karta:\n{PAYMENT_CARD}\n\n"
            f"👤 {PAYMENT_OWNER}\n\n"
            "Summani yozing. Masalan: 10000"
        )
        return

    if data == "send_receipt":
        await receipt_button(update, context)
        return
